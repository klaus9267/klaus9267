<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0,2,2,5,30&height=200&section=header&text=Minho&fontSize=80&fontAlignY=35&desc=BackEnd%20Developer&descAlignY=55&descAlign=50" />
</div>

### 0.1초에 도전하는 개발자 김민호입니다
### BackEnd Developer

---

## About Me

- 🧑‍💻 인덱스, 쿼리 튜닝 등을 통해 캐싱 없이 **조회 성능을 94% 개선**(8.28s → 0.481s 100만 건 기준)한 경험이 있습니다.
- ⚙️ AWS기반 Cloud Server 구축을 경험했으며 **Mini PC기반 On-Premise Server를 구축 및 운영**하고 있습니다.
- 🌐 대중교통 경로탐색 API 비용 절약을 위해 **오픈소스를 활용한 대중교통 경로탐색을 직접 구현**한 경험이 있습니다.
- ☁️ Mini PC·Docker 기반으로 CI/CD, 인프라 자동화, Blue-Green 방식 무중단 배포 등 **운영 환경까지 직접 구축**하고 있습니다.
- 🧠 현재는 **Kotlin + Spring 기반의 App 프로젝트를 운영**하며 서버/인프라 역량을 확장하고 있습니다.
- 🛠️ TypeScript + React 기반의 **통합 관리자 시스템**을 설계하고 직접 운영하며 운영 업무 효율을 극대화했습니다.
- 🎯 사용자 경험과 안정적인 서비스 구조를 동시에 고려하는 **문제 해결 중심 개발자**를 지향합니다.

---

## 🛠 Primary Tech Stack

![Kotlin](https://img.shields.io/badge/Kotlin-007396?style=flat&logo=kotlin&logoColor=white)
![Java](https://img.shields.io/badge/Java-007396?style=flat&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=springboot&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

## 🛠 Secondary Tech Stack

![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat&logo=redis&logoColor=white)
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=flat&logo=prometheus&logoColor=white)
![Loki](https://img.shields.io/badge/Loki-F46800?style=flat&logo=loki&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-F46800?style=flat&logo=grafana&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat&logo=githubactions&logoColor=white)

---

## 📫 Contact

![Tistory Blog](https://img.shields.io/badge/Blog-klaus9267.tistory.com-orange)
![Gmail](https://img.shields.io/badge/Gmail-klaus9267@gmail.com-D14836?style=flat&logo=gmail&logoColor=white)

---

### 📊 GitHub Stats

![](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=klaus9267&theme=tokyonight)
![](https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=klaus9267&theme=tokyonight)
![](https://github-profile-summary-cards.vercel.app/api/cards/stats?username=klaus9267&theme=tokyonight)

### 방문자 수

<img src="https://count.getloli.com/@klaus9267?name=klaus9267&theme=3d-num&padding=6&offset=0&align=top&scale=1&pixelated=1&darkmode=auto"/>

